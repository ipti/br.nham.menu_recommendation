...
export class MenuUpdateComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.menuService.readById(id).subscribe(menuList => {
      ...
      // this.newMenu.items.forEach((hit: { replaceable: string; code: string; },index: number) => {
        this.newMenu.items[index].replaceable = hit.replaceable ? 'Sim' : 'Não';
        const fgs = this.newMenu.items.map(MenuItems.asFormGroup);
  ...
  updateMenu(): void {
    ...
    if (this.checkForm()) {
      ...
      this.menuService.update(this.newMenu).subscribe(() => {
        this.menuService.showMessage('Cardápio atualizado com sucesso!', 'green-snackbar');
        this.router.navigate(['/menu'])
      })
    }
  }
  ...
  completeMenuItems(): void {
    ...
    this.newMenu.items.forEach((hit: { replaceable: any; code: string;},index: number) => {
      if (hit.code) {
      ...
      if (hit.replaceable) {
        ...
        else if (hit.replaceable !== true && hit.replaceable !== false)
